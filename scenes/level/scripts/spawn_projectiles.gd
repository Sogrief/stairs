...
func _process(delta):
	...
	path_follow.progress_ratio = progress_ratio + 0.2 # mise à jour de la position du spawner
	...
	if timer < 0.0:
		...
		projectile_instance.global_position = self.global_position # spécifie la position du projectile égale à celle du canon
